...
class ImgToTxt(object):
    ...
    def to_image(self, txt, name='out.png', color=False):
        tmp = txt.split('\r\n')
        w = len(tmp[0])
        h = len(tmp)
        im = Image.new('RGB', (w*6, h*9), (255, 255, 255))
        fnt = ImageFont.truetype(f'{base_dir}/templates/Courier New.ttf')
        draw = ImageDraw.Draw(im)
        if color:
            for w in range(self.image.size[1]):
                for h in range(self.image.size[0]):
                    rgb = self.image.getpixel((w, h))
                    text = self.get_char(rgb, color=False)
                    draw.text((w, h), text, fnt=fnt, fill=rgb, spacing=0)
        else:
            draw.text((0, 0), txt, font=fnt, fill=0, spacing=0)
...
class GifToTxt(ImgToTxt):
    ...
    def to_json(self, txt, name='out.json', convert=False):
        ...
        for each_txt in txt:
            each_data = []
            lines = each_txt.split('\r\n')
            if convert:
                for line in lines:
                    each_data.append(re.sub(r'\s', '&nbsp;', line))
            else:
                each_data = lines
    ...
    def to_image(self, txt, images, name='out.gif', color=False):
        tmp = txt[0].split('\r\n')
        w = len(tmp[0])
        h = len(tmp)
        fnt = ImageFont.truetype(f'{base_dir}/templates/Courier New.ttf')
            ...
            if color:
                for width in range(image.size[1]):
                    for height in range(image.size[0]):
                        rgb = image.getpixel((width, height))
                        text = self.get_char(rgb, color=False)
                        draw.text((width, height), text, fnt=fnt, fill=rgb, spacing=0)
            else:
                draw.text((0, 0), each_txt, font=fnt, fill=0, spacing=0)
            ims.append(im)
            i += 1
        im = Image.new('RGB', (w*6, h*9), (255, 255, 255))
    ...
    def run(self, text=False, json=False, image=True, html=False, color=False, style='char'):
        ...
        if json:
            self.to_json(txts, f'{name}.json', convert=False)
        if image:
            txts = []
            for image in images:
                txt = self.transform(image, color=False, style=style)
                txts.append(txt)
            self.to_image(txts, images, f'{name}.gif', color=color)
